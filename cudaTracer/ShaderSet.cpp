...
void ShaderSet::createSet(string p_filePath, string p_vsEntry,
                          string p_psEntry) {
...
void ShaderSet::readShader(const string& p_sourceFilePath, uint8_t*& out_data,
                           int& out_size) {
...
void ShaderSet::createVs(uint8_t* p_vsData, int p_vsDataSize) {
    HR(m_deviceHandler->getDevice()->CreateVertexShader(p_vsData, p_vsDataSize,
                                                        NULL, &m_vs));
}

void ShaderSet::createPs(uint8_t* p_psData, int p_psDataSize) {
    HR(m_deviceHandler->getDevice()->CreatePixelShader(p_psData, p_psDataSize,
                                                       NULL, &m_ps));
}