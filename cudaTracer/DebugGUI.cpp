...
DebugGUI::Result DebugGUI::addVar(string p_barName, Types p_type,
    ...
    if (p_permissions == READ_ONLY) {
        if (TwAddVarRO(bar, p_name.c_str(), (TwType)p_type, p_var,
                       p_options.c_str())) {
            result = SUCCESS;
        }
    } else if (p_permissions == READ_WRITE) {
        if (TwAddVarRW(bar, p_name.c_str(), (TwType)p_type, p_var,
                       p_options.c_str())) {
            result = SUCCESS;
        }
    }
...
void DebugGUI::setSize(string p_barName, int p_x, int p_y) {
    int result;
    result =
        TwDefine(stringFromParams(p_barName, "", "size", p_x, p_y).c_str());
}
...
void DebugGUI::terminate() { TwTerminate(); }
