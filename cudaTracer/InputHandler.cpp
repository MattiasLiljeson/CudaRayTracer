...
void InputHandler::update() {
    ...
    if (keystate[DIK_W] & 0x80) keys[W] = true;
    if (keystate[DIK_A] & 0x80) keys[A] = true;
    if (keystate[DIK_S] & 0x80) keys[S] = true;
    if (keystate[DIK_D] & 0x80) keys[D] = true;
    ...
    if (keystate[DIK_F3] & 0x80) keys[F3] = true;
    if (keystate[DIK_F4] & 0x80) keys[F4] = true;

    if (keystate[DIK_ESCAPE] & 0x80) PostMessage(*hWnd, WM_DESTROY, 0, 0);
}

bool InputHandler::getKey(int key) { return keys[key]; }
